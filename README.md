...
- Fallback даты (фев–дек × 1 и 4 ночи) если нет комнат
- Fallback без детей: если с детьми комнат нет — пробует без детей
- IBE API fallback: если Partner API не находит номера, скрипт проверяет TravelLine Booking Engine с учётом минимального срока проживания
...
| `no_rooms` | Отели со статусом `no_rooms` (сразу IBE fallback, без перебора стандартных дат) |
| `all` | Все отели |
...
- `no_rooms` — нет доступных комнат (ни Partner API, ни IBE API не вернули номеров)
- `ibe_only` — номера найдены только через IBE API (Partner API не видит отель), браузерная проверка не проводится

## Логика поиска комнат

При отсутствии комнат скрипт пробует в порядке:

### 1. Partner API (стандартные даты)

| Шаг | Даты | Гости |
|-----|------|-------|
| 1 | Основная дата | С детьми |
| 2 | Fallback-даты (фев–дек × 1н/4н) | С детьми |
| 3 | Основная дата | Без детей |
| 4 | Fallback-даты | Без детей |

### 2. IBE API fallback (TravelLine Booking Engine)

Если Partner API не нашёл номеров, скрипт обращается к IBE API (`ru-ibe.tlintegration.ru`), который используется виджетами бронирования на сайтах отелей.

**Выбор дат с учётом минимального срока проживания:**

```
nearest_available_dates → start_date, end_date
                          ↓
                min_nights = end_date − start_date
```

IBE API возвращает `start_date` и `end_date` — разница между ними и есть **минимальный срок проживания** отеля. Например:
- `start=Feb 13, end=Feb 14` → минимум **1 ночь**
- `start=Feb 07, end=Feb 09` → минимум **2 ночи**
- `start=Feb 07, end=Feb 10` → минимум **3 ночи**

На основе этих дат строятся варианты:

| Вариант | Даты | Описание |
|---------|------|----------|
| 1 | `start → end` | Точные даты IBE (= min stay) |
| 2 | `start → end+1` | На 1 ночь длиннее минимума |
| 3 | `start → end+2` | На 2 ночи длиннее минимума |

Для каждого варианта дат:

| Шаг | API | Гости | Результат |
|-----|-----|-------|-----------|
| 2a | Partner API | С детьми / без | Диплинк на сайт отеля (`api_status=ok`) |
| 2b | IBE API `hotel_availability` | С детьми / без | Диплинк на виджет TL (`api_status=ibe_only`) |

Приоритет: Partner API → IBE API. Partner API предпочтительнее, т.к. даёт диплинк на сайт отеля.

### Режим `--recheck no_rooms`

При перепроверке отелей со статусом `no_rooms` стандартные даты Partner API пропускаются (уже проверялись), скрипт сразу переходит к шагу 2 (IBE API fallback).

### Статус `ibe_only`

Если номера найдены только через IBE API (Partner API не видит отель):
- `api_status = ibe_only`
- Браузерная проверка **не запускается** (диплинк ведёт на виджет TL, а не на сайт отеля)
- `error_details = Отель не в Partner API, диплинк через TL IBE`
